...
class HysCoboler:
    ...
    __identification_division_reigion__ = []
    __environment_division_reigion__    = []
    __data_division_reigion__           = []
    __procedure_division_reigion__      = []
    ...
    def __init__(self, cobol):
        self.cobol = cobol 
        self.__get_division__()
        self.__get_data_structure__()

#{{{ old
    #def __parse_div_old__(self):
    #    pre_lineno = "" 
...
#}}}
    ...
    def __get_division__(self):
        # インスタンス化時に各divisionのpropertiesの行番号の範囲をセットする
        line_cnt = 1
        with open(self.cobol, 'r') as cbl:
            for line in cbl:
                if self.test_reg(PATTERN_IDENTIFICATION_DIVISION, line):
                    self.__identification_division_reigion__.append(line_cnt)
                if self.test_reg(PATTERN_ENVIRONMENT_DIVISION, line):
                    self.__identification_division_reigion__.append(line_cnt-1)
                    self.__environment_division_reigion__.append(line_cnt)
                if self.test_reg(PATTERN_DATA_DIVISION, line):
                    self.__environment_division_reigion__.append(line_cnt-1)
                    self.__data_division_reigion__.append(line_cnt)
                if self.test_reg(PATTERN_PROCEDURE_DIVISION, line):
                    self.__data_division_reigion__.append(line_cnt-1)
                    self.__procedure_division_reigion__.append(line_cnt)
                line_cnt+=1
            self.__procedure_division_reigion__.append(line_cnt)

# {{{
    def __get_data_structure__old_(self):
        line_cnt = 1
        with open(self.cobol, 'r') as cbl:
            for line in cbl:
                if line_cnt >= self.__data_division_reigion__[0] and line_cnt <= self.__data_division_reigion__[1]:
                    m = self.match_reg(self.__pattern_data_item_size1__, line)
                    m = self.match_reg(self.__pattern_data_item_size2__, line) if not m else m
                    m = self.match_reg(self.__pattern_data_item_size3__, line) if not m else m
                    if m:
                        self.data_structure.append(m[1])
                    else:
                        self.data_structure.append('0')
                line_cnt+=1
# }}}

    def __get_data_structure__(self):
        line_cnt = 1
        with open(self.cobol, 'r') as cbl:
            total=0
            for line in cbl:
                if line_cnt >= self.__data_division_reigion__[0] and line_cnt <= self.__data_division_reigion__[1]:
                    ...
                    if m:
                        size=int(m[1])
                        total+=size
                        self.data_structure.append(str(total))
                    else:
                        self.data_structure.append('0')
                line_cnt+=1
            ...
            with open(html_file, 'w') as html:
                with open(self.cobol, 'r') as cbl:
                    for line in cbl:
                        ...
                        if mark == " ":
                            if self.test_reg(self.__pattern_file_description__, rgna):
                            ...
                            if self.test_reg(self.__pattern_block_contains__, rgnb):
                                html.write("<br />" + rgnb + "<br />")
                            if self.test_reg(self.__pattern_file_value_of__, rgnb):
                                html.write(rgnb + "<br />")
                            if self.test_reg(self.__pattern_data_01__, rgna):
                            ...
                            el_level = 0
                            el_size  = 0
                            match = self.match_reg(self.__pattern_data_item_level__, rgnb)
                            if match:
                                ...
                                #else:
                                #    html_resv = [[]]
                                match2 = self.match_reg(self.__pattern_data_item_name__, rgnb.lstrip()[2:])
                                if match2:
                                    el_name = match2.group(1)
                                    match5 = self.match_reg(self.__pattern_data_item_occurs__, rgnb)
                                    if match5:      #OCCURS
                                        occurs_flag = True
                                        ...
                                        repeat = 0
                                    match6 = self.match_reg(self.__pattern_data_item_size3__, rgnb)
                                    if match6:
                                        el_size = int(match6.group(1))
                                        html_main.append([el_class, el_size, el_name, el_level, repeat])
                                        #i = 0
                                        item_type = self.match_reg(self.__pattern_data_item_type1__, rgnb).group(1)
                                        #while i < repeat+1:
                                        ...
                                        occurs_flag = False
                                        continue
                                    match3 = self.match_reg(self.__pattern_data_item_size1__, rgnb)
                                    if match3:
                                        ...
                                        continue
                                    match4 = self.match_reg(self.__pattern_data_item_size2__, rgnb)
                                    if match4:
...
#}}}
                    ...
                    if line[self.__reigion_mark__] == " ":
                        ...
                        #A領域からの文はインデントしない
                            ...
                            if self.__current_division__(line[self.__reigion_lineno__]) == self.__data_division__:
                                if self.test_reg(PATTERN_DATA_ITEM, line[self.__reigion_b__]):
                                #データ項目でのインデント
                            ...
                            #インデント込みで80文字オーバするのを回避
                                ...
                                if self.test_reg(PATTERN_PERIOD, line[self.__reigion_ab__]): 
                                #IF文等によるインデントの後判定
                                    indent = 0
                                else:
    ...
    def test_reg(self, pattern, string):
        pattern = re.compile(pattern)
        if re.search(pattern, string):
            return True
        return False

    def match_reg(self, pattern, string):
        pattern = re.compile(pattern)
